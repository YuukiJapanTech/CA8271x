...
function showDhcpSvr(ip)
{
 ...
 if (document.dhcpd.dhcpdenable[0].checked == true)
  ...
  document.getElementById('displayDhcpSvr').innerHTML=
   ...
   '<table border=0 width="500" cellspacing=4 cellpadding=0>'+
   '<tr>'+
   '<td width="40%"><font size=2><b>DHCP <% multilang("71" "LANG_SERVER"); %> <% multilang("69" "LANG_IP_ADDRESS"); %>:</b></font></td>'+
  ...
  html +=
   ...
   '<td width="30%"><font size=2><b><% multilang("294" "LANG_DNS_OPTION"); %>:</b></td>'+
   '<td width=70%><input type=radio name=dhcpdns value=0 onClick=showdns()><% multilang("295" "LANG_USE_DNS_RELAY"); %>&nbsp;&nbsp;'+
   '<input type=radio name=dhcpdns value=1 onClick=showdns()><% multilang("296" "LANG_SET_MANUALLY"); %>&nbsp;&nbsp;</td>'+
   '</tr></table></div>'+
   ...
   '<input type="button" value="<% multilang("291" "LANG_PORT_BASED_FILTER"); %>" name="macIpTbl" onClick="macIpClick(\'/portBaseFilterDhcp.asp\')">'+
