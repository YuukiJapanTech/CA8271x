...
{
 if (ip.value=="") {
  alert("<% multilang("1613" "LANG_INVALID_IPV4_ADDR_SHOULD_NOT_EMPTY"); %>");
 ...
 if( IsLoopBackIP( ip.value)==1 ) {
  alert("<% multilang("1615" "LANG_INVALID_IPV4_ADDR"); %>");
  ip.value = ip.defaultValue; // Jenny,  Buglist B058, backward default value
  ip.focus();
  return false;
 }
 if ( !checkDigitRange(ip.value,1,0,255) ) {
  alert("<% multilang("1616" "LANG_INVALID_IPV4_ADDR_1ST_DIGIT"); %>");
  ip.value = ip.defaultValue;
  ip.focus();
  return false;
 }
 if ( !checkDigitRange(ip.value,2,0,255) ) {
  alert("<% multilang("1617" "LANG_INVALID_IPV4_ADDR_2ND_DIGIT"); %>");
  ip.value = ip.defaultValue;
  ip.focus();
  return false;
 }
 if ( !checkDigitRange(ip.value,3,0,255) ) {
  alert("<% multilang("1618" "LANG_INVALID_IPV4_ADDR_3RD_DIGIT"); %>");
  ip.value = ip.defaultValue;
  ip.focus();
  return false;
 }
 if ( !checkDigitRange(ip.value,4,1,254) ) {
  alert("<% multilang("1619" "LANG_INVALID_IPV4_ADDR_4TH_DIGIT"); %>");
  ip.value = ip.defaultValue;
  ip.focus();
  return false;
 }
...
function checkNetmask(netmask, checkEmpty)
{
 ...
 if (checkEmpty == 1 && netmask.value == "") {
  alert("<% multilang("1620" "LANG_INVALID_IPV4_SUBNET_SHOULD_NOT_EMPTY"); %>");
  netmask.value = netmask.defaultValue;
  netmask.focus();
  return false;
 }
 if (validateKey(netmask.value) == 0) {
  alert("<% multilang("1621" "LANG_INVALID_IPV4_SUBNET_SHOULD_BE_DECIMAL_NUM"); %>");
  netmask.value = netmask.defaultValue;
  netmask.focus();
  return false;
 }
 for (i=1; i<=4; i++) {
  ...
  if( !(d==0 || d==128 || d==192 || d==224 || d==240 || d==248 || d==252 || d==254 || d==255 )) {
   alert("<% multilang("1622" "LANG_INVALID_IPV4_SUBNET_DIGIT"); %>");
   netmask.focus();
   return false;
  }
 }
...
function checkMac(macAddr, checkEmpty)
{
 ...
 if (checkEmpty == 1 && macAddr.value.length == 0) {
  alert("<% multilang("1623" "LANG_INVALID_MAC_ADDR_SHOULD_NOT_EMPTY"); %>");
  return false;
 }
 if (macAddr.value.length > 0 && macAddr.value.length < 12) {
  alert("<% multilang("1624" "LANG_INVALID_MAC_ADDR_NOT_COMPLETE"); %>");
  macAddr.focus();
  return false;
 }
 ...
 if (macdigit == macAddr.value.length || macAddr.value == "000000000000") {
  alert("<% multilang("1625" "LANG_INVALID_MAC_ADDR"); %>");
  macAddr.focus();
  return false;
 }
 for (i=0; i<macAddr.value.length; i++) {
  if ((macAddr.value.charAt(i) >= '0' && macAddr.value.charAt(i) <= '9') ||
   ...
   (macAddr.value.charAt(i) >= 'A' && macAddr.value.charAt(i) <= 'F') )
   continue;
  alert("<% multilang("1626" "LANG_INVALID_MAC_ADDR_SHOULD_BE"); %>");
  macAddr.focus();
  return false;
 }
 return true;
...
{
 if ( !confirm('<% multilang("1627" "LANG_CONFIRM_DELETE_ONE_ENTRY"); %>') ) {
  return false;
 }
 else
  return true;
...
{
 if ( !confirm('<% multilang("1629" "LANG_CONFIRM_DELETE"); %>') ) {
  return false;
 }
...
function alertInvalid(fieldname, fieldvalue, additional)
{
 if (additional == undefined)
  alert (fieldname + " " + fieldvalue + "<% multilang("1886" "LANG_INVALID"); %>");
 else
  alert (fieldname + " " + fieldvalue + "<% multilang("1886" "LANG_INVALID"); %>" + additional + ".");
}
...
function validateKeyV6Prefix(str)
{
  ...
  //alert("find_flag="+find_flag);
  if ( find_flag != 1 ) {
 alert("<% multilang("1630" "LANG_INVALID_IPV6_PREFIX"); %>");
   return 0;
  }
  return 1;
}
