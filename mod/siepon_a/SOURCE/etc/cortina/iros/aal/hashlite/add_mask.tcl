...
###
### These are all fields with unmasked value, 
### it's better KEEP original and COPY what you need.
###
...
set ret [ ca_aal_hashlite_hashmask_add 0 $hmask $rslt_mask_idx ]
if {$ret != 0} {
	puts "ca_aal_hashlite_hashmask_add() is failed! (ret=$ret)"
} else {
	set mask_idx [ ca_uint32_get $rslt_mask_idx ]
	puts "Hashmask is added to HashLite Engine."
